...
*/
  ...
  */
  ...
  const getSearchVideos = async () => {
    const response = await fetch("https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=25&q=" + SearchQuery + "&type=video&videoCaption=closedCaption&videoDefinition=standard&videoDuration=medium&videoType=any&key=AIzaSyAmlohCuLzqcbIP-ncvA2brZXkkDsG1EO4");
  ...
  const handleListClick = (suggestion) => {
    ...
    // You might also want to trigger a search here:
          ...
          <div className="m-2">
            <ul className="flex flex-col items-start pl-2.5 gap-2.5">
              {querySuggestion.map((item, index) => (
                <div
                  key={index}
                  className="flex items-center gap-2 p-2 shadow-sm min-w-full hover:bg-slate-200 hover:cursor-pointer hover:rounded-lg"
                  onClick={() => handleListClick(item)}  // Attach handler to each item
